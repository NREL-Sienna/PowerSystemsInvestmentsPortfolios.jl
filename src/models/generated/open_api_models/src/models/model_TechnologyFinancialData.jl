# This file was generated by the Julia OpenAPI Code Generator
# Do not modify this file directly. Modify the OpenAPI specification instead.


@doc raw"""TechnologyFinancialData

    TechnologyFinancialData(;
        name=nothing,
        internal=nothing,
        interest_rate=nothing,
        capital_recovery_period=nothing,
        technology_base_year=nothing,
    )

    - name::String : Name of data
    - internal::InfrastructureSystemsInternal : Internal field
    - interest_rate::Float64 : Interest rate
    - capital_recovery_period::Int64 : Capital recovery period (in years) used for determining overnight capital costs from annualized investment costs
    - technology_base_year::Int64 : Year for which all dollar values for this technology are reported. Will be converted to dollars in the portfolio base year.
"""
Base.@kwdef mutable struct TechnologyFinancialData <: OpenAPI.APIModel
    name::Union{Nothing, String} = nothing
    internal = nothing # spec type: Union{ Nothing, InfrastructureSystemsInternal }
    interest_rate::Union{Nothing, Float64} = nothing
    capital_recovery_period::Union{Nothing, Int64} = nothing
    technology_base_year::Union{Nothing, Int64} = nothing

    function TechnologyFinancialData(name, internal, interest_rate, capital_recovery_period, technology_base_year, )
        OpenAPI.validate_property(TechnologyFinancialData, Symbol("name"), name)
        OpenAPI.validate_property(TechnologyFinancialData, Symbol("internal"), internal)
        OpenAPI.validate_property(TechnologyFinancialData, Symbol("interest_rate"), interest_rate)
        OpenAPI.validate_property(TechnologyFinancialData, Symbol("capital_recovery_period"), capital_recovery_period)
        OpenAPI.validate_property(TechnologyFinancialData, Symbol("technology_base_year"), technology_base_year)
        return new(name, internal, interest_rate, capital_recovery_period, technology_base_year, )
    end
end # type TechnologyFinancialData

const _property_types_TechnologyFinancialData = Dict{Symbol,String}(Symbol("name")=>"String", Symbol("internal")=>"InfrastructureSystemsInternal", Symbol("interest_rate")=>"Float64", Symbol("capital_recovery_period")=>"Int64", Symbol("technology_base_year")=>"Int64", )
OpenAPI.property_type(::Type{ TechnologyFinancialData }, name::Symbol) = Union{Nothing,eval(Base.Meta.parse(_property_types_TechnologyFinancialData[name]))}

function check_required(o::TechnologyFinancialData)
    o.name === nothing && (return false)
    true
end

function OpenAPI.validate_property(::Type{ TechnologyFinancialData }, name::Symbol, val)
end
