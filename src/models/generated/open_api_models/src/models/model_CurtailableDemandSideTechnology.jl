# This file was generated by the Julia OpenAPI Code Generator
# Do not modify this file directly. Modify the OpenAPI specification instead.


@doc raw"""CurtailableDemandSideTechnology

    CurtailableDemandSideTechnology(;
        name=nothing,
        power_systems_type=nothing,
        curtailment_cost_mwh=nothing,
        segments=nothing,
        curtailment_cost=nothing,
        voll=nothing,
        ext=nothing,
        internal=nothing,
        max_demand_curtailment=nothing,
        available=nothing,
    )

    - name::String : The technology name
    - power_systems_type::String : maps to a valid PowerSystems.jl for PCM modeling
    - curtailment_cost_mwh::Vector{Float64} : Energy cost of curtailment
    - segments::Vector{Int64} : Demand segment IDs
    - curtailment_cost::Vector{Float64} : Fraction of VOLL for curtailment cost
    - voll::Float64 : value of lost load
    - ext::Dict : Option for providing additional data
    - internal::InfrastructureSystemsInternal : Internal field
    - max_demand_curtailment::Vector{Float64} : percent of demand that can be curtailed in that segment
    - available::Bool : identifies whether the technology is available
"""
Base.@kwdef mutable struct CurtailableDemandSideTechnology <: OpenAPI.APIModel
    name::Union{Nothing, String} = nothing
    power_systems_type::Union{Nothing, String} = nothing
    curtailment_cost_mwh = nothing # spec type: Union{ Nothing, Vector{Float64} }
    segments = nothing # spec type: Union{ Nothing, Vector{Int64} }
    curtailment_cost = nothing # spec type: Union{ Nothing, Vector{Float64} }
    voll::Union{Nothing, Float64} = nothing
    ext::Union{Nothing, Dict} = nothing
    internal = nothing # spec type: Union{ Nothing, InfrastructureSystemsInternal }
    max_demand_curtailment = nothing # spec type: Union{ Nothing, Vector{Float64} }
    available::Union{Nothing, Bool} = nothing

    function CurtailableDemandSideTechnology(name, power_systems_type, curtailment_cost_mwh, segments, curtailment_cost, voll, ext, internal, max_demand_curtailment, available, )
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("name"), name)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("power_systems_type"), power_systems_type)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("curtailment_cost_mwh"), curtailment_cost_mwh)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("segments"), segments)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("curtailment_cost"), curtailment_cost)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("voll"), voll)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("ext"), ext)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("internal"), internal)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("max_demand_curtailment"), max_demand_curtailment)
        OpenAPI.validate_property(CurtailableDemandSideTechnology, Symbol("available"), available)
        return new(name, power_systems_type, curtailment_cost_mwh, segments, curtailment_cost, voll, ext, internal, max_demand_curtailment, available, )
    end
end # type CurtailableDemandSideTechnology

const _property_types_CurtailableDemandSideTechnology = Dict{Symbol,String}(Symbol("name")=>"String", Symbol("power_systems_type")=>"String", Symbol("curtailment_cost_mwh")=>"Vector{Float64}", Symbol("segments")=>"Vector{Int64}", Symbol("curtailment_cost")=>"Vector{Float64}", Symbol("voll")=>"Float64", Symbol("ext")=>"Dict", Symbol("internal")=>"InfrastructureSystemsInternal", Symbol("max_demand_curtailment")=>"Vector{Float64}", Symbol("available")=>"Bool", )
OpenAPI.property_type(::Type{ CurtailableDemandSideTechnology }, name::Symbol) = Union{Nothing,eval(Base.Meta.parse(_property_types_CurtailableDemandSideTechnology[name]))}

function check_required(o::CurtailableDemandSideTechnology)
    o.name === nothing && (return false)
    o.power_systems_type === nothing && (return false)
    o.available === nothing && (return false)
    true
end

function OpenAPI.validate_property(::Type{ CurtailableDemandSideTechnology }, name::Symbol, val)
end
