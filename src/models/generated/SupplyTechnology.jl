...
- `fuel::Vector{ThermalFuels}`: (default: `Vector(ThermalFuels.OTHER)`) Fuel type according to IEA
...
function SupplyTechnology{T}(; base_power, heat_rate_mmbtu_per_mwh=Dict(), outage_factor=1.0, prime_mover_type=PrimeMovers.OT, capital_costs=LinearCurve(0.0), dn_time=0.0, lifetime=100, ramp_dn_percentage=100.0, available=True, co2=Dict(), name, id, initial_capacity=0.0, cofire_start_limits=Dict(), financial_data, start_fuel_mmbtu_per_mw=0.0, operation_costs=ThermalGenerationCost(), fuel=Vector(ThermalFuels.OTHER), power_systems_type, cofire_level_limits=Dict(), internal=InfrastructureSystemsInternal(), ext=Dict(), balancing_topology, region=Vector(), ramp_up_percentage=100.0, base_year=2020, unit_size=0.0, min_generation_percentage=0.0, start_cost_per_mw=0.0, capacity_limits=(min=0, max=1e8), up_time=0.0, ) where T <: PSY.Generator
