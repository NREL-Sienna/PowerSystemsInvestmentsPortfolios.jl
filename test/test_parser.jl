@testset "Test parser" begin
    ...
    portfolio = database_to_portfolio(db_path, 0.07,0.03,0.03,2025; aggregation=PSY.ACBus)
    ...
    ts_data = DataFrame(DBInterface.execute(db, "SELECT * FROM time_series_associations WHERE owner_category = 'Component';"))
