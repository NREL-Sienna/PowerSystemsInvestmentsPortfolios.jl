@testset "Technology and region getters/setters" begin
    ...
    PSIP.set_start_fuel_mmbtu_per_mw!(supply, 1.7)
    PSIP.set_operation_costs!(supply, thermal_cost)
    ...
    PSIP.set_time_limits!(supply, (up=4.0, down=2.0))
    PSIP.set_unit_size!(supply, 12.0)
    PSIP.set_ramp_limits!(supply, (up=0.3, down=0.25))
    ...
    @test PSIP.get_start_fuel_mmbtu_per_mw(supply) == 1.7
    @test PSIP.get_operation_costs(supply) === thermal_cost
    ...
    @test PSIP.get_time_limits(supply) == (up=4.0, down=2.0)
    @test PSIP.get_unit_size(supply) == 12.0
    @test PSIP.get_ramp_limits(supply) == (up=0.3, down=0.25)
    @test PSIP.get_capacity_limits(supply) == (min=5.0, max=600.0)
    ...
    PSIP.set_capacity_limits_charge!(storage, (min=3.0, max=400.0))
    PSIP.set_storage_tech!(storage, StorageTech.LIB)
    PSIP.set_duration_limits!(storage, (min=2.0, max=12.0))
    PSIP.set_losses!(storage, 0.02)
    PSIP.set_capital_costs_energy!(storage, storage_energy_curve)
    PSIP.set_financial_data!(storage, tech_financial_data)
    PSIP.set_operation_costs!(storage, storage_cost)
    ...
    PSIP.set_capacity_limits_energy!(storage, (min=10.0, max=1000.0))
    PSIP.set_unit_size_energy!(storage, 20.0)
    PSIP.set_unit_size_charge!(storage, 7.5)
    PSIP.set_efficiency!(storage, (in=0.94, out=0.91))
    PSIP.set_unit_size_discharge!(storage, 8.5)
    PSIP.set_capacity_limits_discharge!(storage, (min=6.0, max=500.0))
    PSIP.set_capital_costs_charge!(storage, storage_charge_curve)
    PSIP.set_capital_costs_discharge!(storage, storage_discharge_curve)

    @test PSIP.get_requirements(storage) === storage_req
    @test PSIP.get_prime_mover_type(storage) == PrimeMovers.BT
    @test PSIP.get_lifetime(storage) == 28
    ...
    @test PSIP.get_capacity_limits_charge(storage) == (min=3.0, max=400.0)
    @test PSIP.get_storage_tech(storage) == StorageTech.LIB
    @test PSIP.get_duration_limits(storage) == (min=2.0, max=12.0)
    @test PSIP.get_losses(storage) == 0.02
    @test PSIP.get_capital_costs_energy(storage) === storage_energy_curve
    @test PSIP.get_financial_data(storage) === tech_financial_data
    @test PSIP.get_operation_costs(storage) === storage_cost
    ...
    @test PSIP.get_capacity_limits_energy(storage) == (min=10.0, max=1000.0)
    @test PSIP.get_unit_size_energy(storage) == 20.0
    @test PSIP.get_unit_size_charge(storage) == 7.5
    @test PSIP.get_efficiency(storage) == (in=0.94, out=0.91)
    @test PSIP.get_unit_size_discharge(storage) == 8.5
    @test PSIP.get_capacity_limits_discharge(storage) == (min=6.0, max=500.0)
    @test PSIP.get_capital_costs_charge(storage) === storage_charge_curve
    @test PSIP.get_capital_costs_discharge(storage) === storage_discharge_curve

    # Also test optional fields can be set back to nothing.
    PSIP.set_capacity_limits_charge!(storage, nothing)
    PSIP.set_unit_size_charge!(storage, nothing)
    PSIP.set_capital_costs_charge!(storage, nothing)
    @test isnothing(PSIP.get_capacity_limits_charge(storage))
    @test isnothing(PSIP.get_unit_size_charge(storage))
    @test isnothing(PSIP.get_capital_costs_charge(storage))
    ...
    PSIP.set_value_of_lost_load!(demand_requirement, 1200.0)
    ...
    PSIP.set_unserved_demand_curve!(demand_requirement, demand_req_curve)
    PSIP.set_new_construction_year!(demand_requirement, 2035)
    PSIP.set_new_demand_mw!(demand_requirement, 25.0)
    ...
    @test PSIP.get_value_of_lost_load(demand_requirement) == 1200.0
    ...
    @test PSIP.get_unserved_demand_curve(demand_requirement) === demand_req_curve
    @test PSIP.get_new_construction_year(demand_requirement) == 2035
    @test PSIP.get_new_demand_mw(demand_requirement) == 25.0
    ...
    PSIP.set_price_per_unit!(demand_side, demand_side_price)
    PSIP.set_name!(demand_side, "demand_side_updated")
    PSIP.set_id!(demand_side, 140)
    PSIP.set_available!(demand_side, false)
    PSIP.set_shift_variable_cost!(demand_side, demand_side_shift_cost)
    PSIP.set_curtailment_cost!(demand_side, demand_side_curtailment)
    PSIP.set_technology_efficiency!(demand_side, 0.75)
    PSIP.set_max_demand_advance!(demand_side, 6.0)
    PSIP.set_demand_energy_efficiency!(demand_side, 0.92)
    PSIP.set_max_demand_curtailment!(demand_side, 0.2)
    PSIP.set_max_demand_delay!(demand_side, 8.0)
    ...
    @test PSIP.get_peak_demand_mw(demand_side) == 50.0
    ...
    PSIP.set_capital_costs!(aggregate_transport, aggregate_capital)
    ...
    PSIP.set_unit_size!(aggregate_transport, 3.0)
    PSIP.set_available!(aggregate_transport, false)
    PSIP.set_line_loss!(aggregate_transport, 0.03)
    PSIP.set_capacity_limits!(aggregate_transport, (min=50.0, max=1500.0))
    ...
    @test PSIP.get_capital_costs(aggregate_transport) === aggregate_capital
    ...
    @test PSIP.get_unit_size(aggregate_transport) == 3.0
    @test !PSIP.get_available(aggregate_transport)
    @test PSIP.get_line_loss(aggregate_transport) == 0.03
    @test PSIP.get_capacity_limits(aggregate_transport) == (min=50.0, max=1500.0)
    ...
    PSIP.set_capital_costs!(nodal_ac_transport, nodal_ac_capital)
    ...
    PSIP.set_reactance!(nodal_ac_transport, 2.5)
    PSIP.set_resistance!(nodal_ac_transport, 1.4)
    PSIP.set_voltage!(nodal_ac_transport, 345.0)
    PSIP.set_unit_size!(nodal_ac_transport, 4.0)
    PSIP.set_capacity_limits!(nodal_ac_transport, (min=10.0, max=800.0))

    @test PSIP.get_requirements(nodal_ac_transport) === nodal_ac_requirements
    @test PSIP.get_capital_costs(nodal_ac_transport) === nodal_ac_capital
    ...
    @test PSIP.get_reactance(nodal_ac_transport) == 2.5
    @test PSIP.get_resistance(nodal_ac_transport) == 1.4
    @test PSIP.get_voltage(nodal_ac_transport) == 345.0
    @test PSIP.get_unit_size(nodal_ac_transport) == 4.0
    @test PSIP.get_capacity_limits(nodal_ac_transport) == (min=10.0, max=800.0)
    ...
    PSIP.set_capital_costs!(nodal_hvdc_transport, nodal_hvdc_capital)
    ...
    PSIP.set_unit_size!(nodal_hvdc_transport, 5.0)
    PSIP.set_line_loss!(nodal_hvdc_transport, nodal_hvdc_line_loss)
    PSIP.set_capacity_limits!(nodal_hvdc_transport, (min=30.0, max=1800.0))

    @test PSIP.get_requirements(nodal_hvdc_transport) === nodal_hvdc_requirements
    @test PSIP.get_capital_costs(nodal_hvdc_transport) === nodal_hvdc_capital
    ...
    @test PSIP.get_unit_size(nodal_hvdc_transport) == 5.0
    @test PSIP.get_line_loss(nodal_hvdc_transport) === nodal_hvdc_line_loss
    @test PSIP.get_capacity_limits(nodal_hvdc_transport) == (min=30.0, max=1800.0)
    ...
    colocated_requirements = Requirement[req_a]
    ...
    PSIP.set_requirements!(colocated_supply_storage, colocated_requirements)
    PSIP.set_operation_costs_power!(colocated_supply_storage, colocated_power_cost)
    PSIP.set_lifetime_storage!(colocated_supply_storage, 26)
    PSIP.set_operation_costs_solar!(colocated_supply_storage, colocated_solar_cost)
    PSIP.set_capacity_limits_wind!(colocated_supply_storage, (min=10.0, max=900.0))
    PSIP.set_name!(colocated_supply_storage, "colocated_supply_storage_updated")
    PSIP.set_capital_costs_power!(colocated_supply_storage, colocated_capital_power)
    PSIP.set_capacity_power_limits!(colocated_supply_storage, (min=5.0, max=600.0))
    PSIP.set_lifetime_wind!(colocated_supply_storage, 30)
    PSIP.set_capacity_energy_limits!(colocated_supply_storage, (min=20.0, max=1200.0))
    PSIP.set_duration_limits!(colocated_supply_storage, (min=2.0, max=10.0))
    PSIP.set_min_inverter_capacity!(colocated_supply_storage, 15.0)
    PSIP.set_id!(colocated_supply_storage, 180)
    PSIP.set_operation_costs_energy!(colocated_supply_storage, colocated_energy_cost)
    PSIP.set_capital_costs_energy!(colocated_supply_storage, colocated_capital_energy)
    PSIP.set_operation_costs_inverter!(colocated_supply_storage, colocated_inverter_cost)
    ...
    PSIP.set_capacity_limits_solar!(colocated_supply_storage, (min=12.0, max=950.0))
    PSIP.set_internal!(colocated_supply_storage, colocated_internal)
    PSIP.set_operation_costs_wind!(colocated_supply_storage, colocated_wind_cost)
    ...
    PSIP.set_capital_costs_wind!(colocated_supply_storage, colocated_capital_wind)
    PSIP.set_lifetime_solar!(colocated_supply_storage, 29)
    PSIP.set_capital_costs_inverter!(colocated_supply_storage, colocated_capital_inverter)
    PSIP.set_max_inverter_capacity!(colocated_supply_storage, 450.0)
    PSIP.set_capital_costs_solar!(colocated_supply_storage, colocated_capital_solar)

    @test PSIP.get_requirements(colocated_supply_storage) === colocated_requirements
    @test PSIP.get_operation_costs_power(colocated_supply_storage) === colocated_power_cost
    @test PSIP.get_lifetime_storage(colocated_supply_storage) == 26
    @test PSIP.get_operation_costs_solar(colocated_supply_storage) === colocated_solar_cost
    @test PSIP.get_capacity_limits_wind(colocated_supply_storage) == (min=10.0, max=900.0)
    @test PSIP.get_name(colocated_supply_storage) == "colocated_supply_storage_updated"
    @test PSIP.get_capital_costs_power(colocated_supply_storage) === colocated_capital_power
    @test PSIP.get_capacity_power_limits(colocated_supply_storage) == (min=5.0, max=600.0)
    @test PSIP.get_lifetime_wind(colocated_supply_storage) == 30
    @test PSIP.get_capacity_energy_limits(colocated_supply_storage) ==
          (min=20.0, max=1200.0)
    @test PSIP.get_duration_limits(colocated_supply_storage) == (min=2.0, max=10.0)
    @test PSIP.get_min_inverter_capacity(colocated_supply_storage) == 15.0
    @test PSIP.get_id(colocated_supply_storage) == 180
    @test PSIP.get_operation_costs_energy(colocated_supply_storage) ===
          colocated_energy_cost
    @test PSIP.get_capital_costs_energy(colocated_supply_storage) ===
          colocated_capital_energy
    @test PSIP.get_operation_costs_inverter(colocated_supply_storage) ===
    ...
    @test PSIP.get_capacity_limits_solar(colocated_supply_storage) == (min=12.0, max=950.0)
    @test PSIP.get_internal(colocated_supply_storage) === colocated_internal
    @test PSIP.get_operation_costs_wind(colocated_supply_storage) === colocated_wind_cost
    ...
    @test PSIP.get_capital_costs_wind(colocated_supply_storage) === colocated_capital_wind
    @test PSIP.get_lifetime_solar(colocated_supply_storage) == 29
    @test PSIP.get_capital_costs_inverter(colocated_supply_storage) ===
          colocated_capital_inverter
    @test PSIP.get_max_inverter_capacity(colocated_supply_storage) == 450.0
    @test PSIP.get_capital_costs_solar(colocated_supply_storage) === colocated_capital_solar
end
